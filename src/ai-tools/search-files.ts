...
import type { AgentTool, ChunkMetadata, SearchMatch, ToolContext, ToolResult } from './types.js'
...
async function performRegexSearch(
  ...
  try {
    ...
    const matches: SearchMatch[] = await context.storage.searchFiles(pattern, {
      path: normalizedPath,
      filePattern,
      caseSensitive,
      maxResults,
...
export const searchFilesTool: AgentTool = {
  name: 'search_files',
  description:
    'Search for a pattern in files. Returns matching lines with file paths and line numbers. Use this to find specific code, function definitions, or usages.',
  inputSchema: {
    ...
    properties: {
      pattern: {
        type: 'string',
        description:
          'The search pattern (supports regex). Examples: "function handleSubmit", "import.*from", "TODO:"',
      },
      path: {
        type: 'string',
        description:
          'The relative path to search in (e.g., "src" to search only in src/). Use "" or "." for entire project.',
      },
      filePattern: {
        type: 'string',
        description:
          'Optional glob pattern to filter files to search (e.g., "*.ts", "*.tsx", "**/*.test.ts")',
      },
      semantic: {
        type: 'boolean',
        description:
          'Use semantic search (RAG) instead of regex. Requires ragRetriever in context. Defaults to false.',
      },
      caseSensitive: {
        type: 'boolean',
  ...
  execute(input, context) {
    ...
    const semantic = (input.semantic as boolean) ?? false
