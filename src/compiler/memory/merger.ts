...
import { type RotationLogEntry, applyDirectives } from './directives.js'
import { memoryMergeYamlSchema } from './directives.schema.js'
import { parseDocument, serializeDocument } from './precedence.js'
import { type ProvenanceMap, buildProvenance } from './provenance.js'
...
function isShallowRepository(cwd: string | undefined): boolean {
  try {
...
function tryReadFile(filePath: string, warnings: string[]): string | undefined {
  try {
...
export async function mergeAgentsMd(opts: MergeOptions): Promise<MergeResult> {
  const warnings: string[] = []
  const rotationLog: RotationLogEntry[] = []
  ...
  if (parsedLayers.length === 0) {
    const empty: ProvenanceMap = buildProvenance([], opts.layers.slice())
    return { content: '', provenance: empty, rotationLog: [], warnings }
  }

  // Step 2: Start with base layer sections
  const mergedSections = new Map<string, { heading: string; content: string }>()
  ...
  if (opts.directivesPath) {
    ...
    if (directivesContent !== undefined) {
      const rawYaml = parseYaml(directivesContent) as unknown
