...
const DEFAULT_REQUIRED_SECTIONS: readonly VisionRequiredSection[] = [
  ...
  if (!lastUpdated) {
    ...
    if (staleAfter > 0) {
      ...
      if (ageDays > staleAfter) {
        console.warn(
          `[vision-warn] ${relativeName}: last_updated is ${ageDays} days old (>${staleAfter}). Consider refreshing.`,
        )
      }
    }
  }
