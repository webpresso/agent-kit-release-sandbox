  ...
  create(context) {
    return {
      CallExpression(node) {
        ...
        if (!name) return
        if (!RESOLVE_FUNCTIONS.has(name) && !QUALIFIED_RESOLVE.has(name)) return
  ...
  create(context) {
    return {
      CallExpression(node) {
        ...
        const first = args[0]
        if (!isDunderDirname(first) && !isMetaDirname(first)) return
        ...
        for (let i = 1; i < args.length; i++) {
          ...
          if (/\.\./.test(arg.value) && MONOREPO_DIRS.test(arg.value)) {
            context.report({
              node: arg,
              message:
                'Cross-package path traversal via __dirname. Use proper package imports or findRepoRoot() instead.',
            })
            return
          }
        }
        ...
        const hasMonoDir = args.some(
          (a) => a.type === 'Literal' && typeof a.value === 'string' && MONOREPO_DIRS.test(a.value),
        )
