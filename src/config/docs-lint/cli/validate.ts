#!/usr/bin/env bun
import type { ValidateOptions } from './commands/validate-command.js'
...
function parseArgs(argv: string[]): ValidateOptions & { help: boolean } {
  const files: string[] = []
  const options: ValidateOptions & { help: boolean } = { help: false }
