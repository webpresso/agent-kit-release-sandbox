 ...
 */
export function emitConstitution(parsed: ParsedBlueprintForDb, repoRoot: string): string {
  ...
  const sections: string[] = ['# Repository Constitution', '']
...
function readFirstSection(filePath: string): string | null {
  try {
...
function readFirstBulletListOrSection(filePath: string): string | null {
  try {
    const content = readFileSync(filePath, 'utf-8')
