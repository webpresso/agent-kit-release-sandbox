...
import { planFrontmatterSchema, taskStatusSchema, type BlueprintTaskStatus } from '#core/schema'
...
function parseWithSchema<T extends z.ZodType<BlueprintFrontmatter>>(
  markdown: string,
  schema: T,
...
export function parseBlueprint(markdown: string, name: string): Blueprint {
...
export function serializeBlueprint(blueprint: Blueprint): string {
  ...
  if (blueprint.status) data.status = blueprint.status
  if (blueprint.complexity) data.complexity = blueprint.complexity
  if (blueprint.lastUpdated) data.last_updated = blueprint.lastUpdated
...
function extractExplicitTaskStatus(section: string): BlueprintTaskStatus | undefined {
...
function validateTaskFormat(content: string): void {
  const wrongFormat = content.match(/^###\s+(?:\[[^\]]+\]\s+)?Task\s+\d+(?:\.\d+)+:/gm)
  if (wrongFormat && wrongFormat.length > 0) {
    throw new Error(
      `Plan parsing failed: Found ${wrongFormat.length} task(s) using '### Task' (3 hashes). ` +
...
function extractTasks(content: string): Task[] {
  ...
  return matches.map((match, index) => {
    ...
    return Object.assign(
      ...
      explicitStatus && { statusExplicit: true },
      depends.length > 0 && { depends },
      blockedReason && { blockedReason },
      description && { description },
      { stepType: inferTaskType(description || (match[2] ?? '')) },
      extractTaskMetadata(description || (match[2] ?? '')),
      extractComplexity(description || ''),
    )
  })
}

function requiresExplicitTaskStatus(blueprintStatus: string): boolean {
  return (
...
function assertExplicitTaskStatuses(
  tasks: Task[],
  ...
  if (missing.length > 0) {
    throw new Error(
      `Blueprint ${blueprintName} requires explicit **Status:** on every task. Missing: ${missing.join(', ')}`,
    )
  }
}

const TASK_TYPE_RULES: Array<{ keywords: string[]; type: string }> = [
...
function inferTaskType(desc: string): string {
...
function extractTaskMetadata(desc: string): { targetPackage?: string; targetFile?: string } {
  const result: { targetPackage?: string; targetFile?: string } = {}
...
function extractComplexity(desc: string): { complexity?: string } {
...
function extractDepends(section: string): string[] {
...
function extractBlocked(section: string): string | undefined {
...
function extractTaskDescription(section: string): string | undefined {
...
function extractPhases(content: string, allTasks: Task[]): Phase[] {
