...
export type BundleBudgetViolationKind =
  | 'js-asset-too-large'
  | 'html-eager-js-asset-too-large'
  | 'html-eager-js-total-too-large'
  | 'html-referenced-asset-missing'
...
const MODULEPRELOAD_HREF_PATTERN =
  /<link\b(?=[^>]*\brel\s*=\s*(["'])modulepreload\1)[^>]*\bhref\s*=\s*(["'])(.*?)\2[^>]*>/gi

export function analyzeBundleBudget(options: AnalyzeBundleBudgetOptions): BundleBudgetResult {
  const limits: BundleBudgetLimits = {
  ...
  const assets = normalizeAssets(options.assets).filter(
    (asset) => !isIgnored(asset.path, options.ignore),
  )
  ...
  const htmlEagerJsAssets: BundleBudgetAsset[] = []
  ...
  for (const reference of htmlEagerJsReferences) {
    if (isIgnored(reference, options.ignore)) continue
    ...
    if (
      limits.maxHtmlEagerJsAssetBytes !== undefined &&
  ...
  if (
    limits.maxHtmlEagerJsTotalBytes !== undefined &&
...
export function extractHtmlEagerJsReferences(html: string): string[] {
  const references = new Set<string>()

  for (const match of html.matchAll(SCRIPT_SRC_PATTERN)) {
    const src = match[2]
    if (src) addJavaScriptReference(references, src)
  }

  for (const match of html.matchAll(MODULEPRELOAD_HREF_PATTERN)) {
    const href = match[3]
    if (href) addJavaScriptReference(references, href)
  }

  return [...references]
}

export function formatBundleBudgetReport(result: BundleBudgetResult): string {
  const lines: string[] = []
...
function addJavaScriptReference(references: Set<string>, rawReference: string): void {
  const normalized = normalizeReferencePath(rawReference)
  if (normalized && isJavaScriptPath(normalized)) references.add(normalized)
}

function normalizeAssets(assets: readonly BundleBudgetAsset[]): BundleBudgetAsset[] {
...
function normalizeReferencePath(rawPath: string): string {
  const withoutQuery = rawPath.split(/[?#]/, 1)[0] ?? ''
  return withoutQuery.replace(/^\.?\//, '').replace(/^\//, '')
}

function isJavaScriptPath(assetPath: string): boolean {
  return JS_EXTENSION_PATTERN.test(assetPath)
}

function isIgnored(assetPath: string, ignore: readonly (RegExp | string)[] | undefined): boolean {
  if (!ignore?.length) return false
  return ignore.some((pattern) => {
    if (typeof pattern === 'string') return assetPath.includes(pattern)
    return pattern.test(assetPath)
  })
}

function compareAssetPath(left: BundleBudgetAsset, right: BundleBudgetAsset): number {
