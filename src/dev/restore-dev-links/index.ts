...
export type RestoreOutcome =
  | { kind: 'no-state-file' }
  | { kind: 'invalid-state-file'; reason: string }
  | { kind: 'source-missing'; expectedSource: string }
  | { kind: 'already-linked'; target: string; source: string }
  | { kind: 'relinked'; target: string; source: string; previous: string | null }
...
export function restoreDevLinks(options: RestoreOptions = {}): RestoreResult {
  ...
  if (!existsSync(join(source, 'package.json'))) {
    stderr.write(
      `wp-restore-dev-links: state file points at ${source} but no package.json found there.\n` +
...
function ensureSymlink(
  target: string,
  source: string,
): Extract<RestoreOutcome, { kind: 'already-linked' | 'relinked' }> {
