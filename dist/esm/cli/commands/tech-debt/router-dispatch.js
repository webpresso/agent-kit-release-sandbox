...
const SUPPORTED_FROM_AUDIT_NAMES = ['skill-sizes', 'broken-refs', 'memory-rotation'];
function isSupportedAuditName(name) {
    return SUPPORTED_FROM_AUDIT_NAMES.includes(name);
}
/**
 * Run the named audit and extract findings.
 */
async function runAuditForTechDebt(auditName, cwd) {
    switch (auditName) {
        case 'skill-sizes': {
 ...
 */
function computeAutoFiledHash(auditName, findings) {
 ...
 */
function findExistingByHash(techDebtRoot, hash) {
    for (const statusDir of STATUS_DIRS) {
        const dir = path.join(techDebtRoot, statusDir);
        if (!existsSync(dir))
            continue;
 ...
 */
function auditNameToSeverity(auditName) {
    switch (auditName) {
        case 'skill-sizes':
            return 'medium';
        case 'broken-refs':
 ...
 */
function generateFromAuditContent(title, options) {
    const today = new Date().toISOString().slice(0, 10);
    const linkedBlueprintsYaml = options.linkedBlueprints.length > 0
...
async function handleNewFromAudit(auditName, options) {
    if (!isSupportedAuditName(auditName)) {
        throw new Error(`Unknown audit name: ${auditName}. Supported: ${SUPPORTED_FROM_AUDIT_NAMES.join(', ')}`);
    }
    ...
    const reviewCadence = 'biweekly';
    const status = 'needs-remediation';
    const today = new Date().toISOString().slice(0, 10);
