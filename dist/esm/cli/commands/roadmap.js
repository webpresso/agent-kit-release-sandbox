...
export function formatRoadmapDetails(result, childResults) {
    const childLines = childResults.length > 0
        ? childResults.map((child) => `- ${child.slug} status=${child.blueprint.status} tasks=${child.blueprint.tasks.filter((task) => task.status === 'done').length}/${child.blueprint.tasks.length} done`)
        : ['- No child blueprints declared'];
    const blockerLines = childResults.flatMap((child) => child.blueprint.tasks
    ...
    cli
        ...
        switch (subcommand) {
            ...
            case 'show': {
                ...
                const roadmapNode = model.roadmaps.find((entry) => entry.roadmap.name === result.slug ||
                ...
                printBlueprintOutput(options.json
                    ? { ...result, children: childResults }
                    : formatRoadmapDetails(result, childResults), options.json);
                return;
            }
            default:
