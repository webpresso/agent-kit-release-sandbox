 ...
 */
export async function dbBuild(projectRoot) {
...
export async function dbVerify(projectRoot) {
    ...
    try {
        ...
        const techDebtRows = conn.db
            .prepare('SELECT slug, file_path, content_hash FROM tech_debt_items')
            .all();
 ...
 */
export async function dbQuery(projectRoot, templateId, params = {}) {
 ...
 */
export function dbBrowse(projectRoot, _execSync = execSync) {
    const dbPath = agentDbPath(projectRoot);
    ...
    try {
        ...
        process.stderr.write('Datasette is not installed. Install it with:\n\n  pip install datasette\n\n');
        process.exit(1);
    }
...
// ---------------------------------------------------------------------------
export async function executeBlueprintDbSubcommand(verb, args, options, print) {
    ...
    switch (verb) {
        ...
        case 'query': {
            ...
            if (options.params) {
                try {
                    ...
                    throw new Error(`Invalid --params JSON: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
        ...
        default: {
            throw new Error(`Unknown blueprint db verb: ${verb ?? '(none)'}\n\nUse one of: build, query, verify, browse`);
        }
    }
}
