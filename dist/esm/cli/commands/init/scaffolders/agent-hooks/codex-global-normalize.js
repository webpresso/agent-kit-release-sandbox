...
const MANAGED_CONTEXT_MODE_GLOBAL_HOOK_BASENAME_SET = new Set(MANAGED_CONTEXT_MODE_GLOBAL_HOOK_BASENAMES);
...
function normalizeGlobalCodexHookCommand(command, options, managedHooksDir) {
    if (typeof command !== 'string')
        return command;
    const trimmed = command.trim();
    const managedContextModePath = managedHooksDir && options.contextModeBinary
...
function launcherForCommand(command, options, managedHooksDir) {
...
function renderShellLauncher(parts) {
    return `#!/bin/sh\nexec ${parts.join(' ')}\n`;
}
function contextModeManagedLauncherPath(command, managedHooksDir) {
...
function omxManagedLauncherPath(command, managedHooksDir) {
    if (parseOmxHookCommand(command) === null)
        return null;
    return join(managedHooksDir, MANAGED_OMX_GLOBAL_HOOK_BASENAME);
}
function contextModeLauncherBasename(subcommand) {
    return `wp-global-codex-context-mode-${subcommand}.sh`;
}
function parseContextModeHookCommand(command) {
...
export function isManagedContextModeGlobalLauncherBasename(basenameValue) {
