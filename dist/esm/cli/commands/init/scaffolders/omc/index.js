 ...
 */
export function ensureOmc(input) {
    const scope = input.scope ?? 'user';
    if (input.options.dryRun)
        return { kind: 'omc-skipped-dry-run', scope };
    if (process.env.WP_SKIP_OMC === '1')
        return { kind: 'omc-skipped-opt-out', scope };
    const commandExists = input.commandExists ?? defaultCommandExists;
    if (!commandExists('claude'))
        return { kind: 'omc-skipped-no-cli', scope };
