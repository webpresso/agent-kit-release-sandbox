...
function getSectionLastTouchedAgeMs(filePath, cwd) {
    try {
...
export function applyDirectives(sections, directives, context) {
    ...
    for (const directive of directives) {
        ...
        if (directive.op === 'rotate') {
            if (context.isShallowClone) {
                context.warnings.push(`rotate skipped for section "${directive.heading}": shallow clone detected`);
                continue;
            }
            // rotation_eligible must be explicitly true; skip if not set
            if (directive.rotation_eligible !== true)
                continue;
