...
export class SessionMemorySessionStore {
    ...
    captureEvent(input) {
        ...
        this.db
            .prepare(`INSERT OR REPLACE INTO session_events (session_id, event_id, repo_hash, ts, tool_name, content)
         VALUES (?, ?, ?, ?, ?, ?)`)
    ...
    snapshot(input) {
        ...
        const sessionId = input.sessionId ?? `${input.repoHash}:${input.agentId ?? 'default'}`;
        const rows = this.db
            .prepare(`SELECT session_id, event_id, ts, tool_name, content
         FROM session_events
        ...
        const status = included.length < rows.length ? 'partial' : 'complete';
    ...
    restore(input) {
        ...
        if (!query)
            return [];
        const rows = this.db
            .prepare(`SELECT e.session_id, e.event_id, e.ts, e.tool_name, e.content, bm25(session_events_fts) * -1 AS score
