import { createBlockedResult, findDuplicateFunctions, getSharedFunctions, } from '#quality-engine/package-import-rules';
...
export function validatePackageImports(input, options = {}) {
    ...
    if (!IMPL_EXTENSIONS.test(filePath))
        return { validator: VALIDATOR_NAME, passed: true };
    const sharedFunctions = getSharedFunctions(profile);
    if (sharedFunctions.some((fn) => filePath.includes(`/${fn.source}/`) ||
