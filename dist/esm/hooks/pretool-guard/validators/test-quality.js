...
export const MUTATION_GAMING_PATTERNS = [
    {
...
export const TAUTOLOGICAL_PATTERNS = [
    {
...
export function findTautologicalAssertions(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (const { pattern, description } of TAUTOLOGICAL_PATTERNS) {
            ...
            if (match)
                matches.push({ line: i + 1, pattern: description, match: match[0] });
        }
    }
    return matches;
}
export function findMutationGamingPatterns(content, filePath) {
    const matches = [];
    if (filePath) {
        for (const { pattern, description, fileLevel } of MUTATION_GAMING_PATTERNS) {
            ...
            if (match)
                matches.push({ line: 0, pattern: description, match: match[0] });
        }
    }
    const lines = content.split('\n');
    for (let i = 0; i < lines.length; i++) {
...
export function validateTestQuality(input) {
