 ...
 */
export function isBlueprintPath(filePath, blueprintsRoot) {
    const normalized = normalizePlanningPath(filePath);
    if (blueprintsRoot !== undefined)
        return matchesRoot(normalized, blueprintsRoot);
    return CANONICAL_BLUEPRINTS_ROOTS.some((root) => matchesRoot(normalized, root));
}
export function getNonCanonicalPlanningPathViolation(filePath, blueprintsRoot, techDebtRoot) {
    const normalized = normalizePlanningPath(filePath);
    ...
    if (bpRoots.some((r) => matchesRoot(normalized, r)) ||
...
//# sourceMappingURL=path-contract.js.map