...
function isValidImportPath(importPath) {
    ...
    const looksLikeFilePath = importPath.includes('.') || importPath.startsWith('./') || importPath.startsWith('../');
    // Skip if it looks like an npm package (@org/package)
    // But allow paths starting with a dot (like .agent/...)
    const looksLikeNpmPackage = importPath.includes('/') &&
 ...
 */
function tryExtractImport(trimmedLine) {
 ...
 */
function processLineForImport(line, lineNumber, inCodeBlock) {
 ...
 */
export function resolveImportPath(importPath, fromFile, projectRoot) {
 ...
 */
function detectCircularDeps(filePath, _projectRoot, visited, chain) {
 ...
 */
export function validateImports(filePath, content, projectRoot) {
    const errors = [];
