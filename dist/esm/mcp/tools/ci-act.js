...
function publicCommandDetails(input) {
    const command = sanitizePublicCiActArgv(buildPublicCiActCommand(input));
    return { command: command.command, args: [...command.args] };
}
const tool = {
    name: 'wp_ci_act',
    description: 'Run local GitHub Actions workflows through `act` via the public secret contract (`wp config secrets ...`, then `with-secrets -- act ...`).',
    inputSchema,
    outputSchema,
    annotations: {
        title: 'CI act',
