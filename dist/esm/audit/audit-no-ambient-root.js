...
const DIRECT_CALL_PATTERNS = [
    {
        ...
        message: 'Direct call to findRepoRoot() detected. Pass workspace root as an explicit argument instead.',
    },
    {
        re: /\bfindRootSync\s*\(/,
        pattern: 'findRootSync(',
        message: 'Direct call to findRootSync() detected. Pass workspace root as an explicit argument instead.',
    },
    {
        re: /\bfindProjectRoot\s*\(/,
        pattern: 'findProjectRoot(',
        message: 'Direct call to findProjectRoot() detected. Pass workspace root as an explicit argument instead.',
    },
];
...
function detectInFile(filePath, contents) {
    const violations = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (TOP_LEVEL_ROOT_CONST_RE.test(line)) {
            violations.push({
                ...
                message: 'Module-scope root constant detected. Root resolution at import time crashes test workers. Move into a function and pass root as an explicit parameter.',
            });
        }
        ...
        if (DEFAULT_CWD_PARAM_RE.test(line) && /\bfunction\b|\(.*:.*string\s*=/.test(line)) {
            violations.push({
                ...
                line: lineNumber,
                pattern: '= process.cwd()',
                message: 'Default `= process.cwd()` parameter in function signature detected. Library functions must accept an explicit root argument; reserve process.cwd() for CLI entry points.',
            });
        }
    }
    return violations;
 ...
 */
export function detectAmbientRoot(files) {
    const violations = [];
...
//# sourceMappingURL=audit-no-ambient-root.js.map