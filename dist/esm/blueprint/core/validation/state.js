 ...
 */
function validateCompletedFolder(status, isInCompleted, criteria) {
    if (status === 'completed' && !isInCompleted) {
        return {
            valid: false,
            error: 'Plan has status: completed but is not in completed/ folder. Move it with: git mv <path> webpresso/blueprints/completed/',
        };
    }
...
function validateDraftFolder(status, isInDraft, criteria) {
    if (status === 'draft' && !isInDraft) {
        return {
            valid: false,
            error: 'Plan has status: draft but is not in draft/ folder. Move it with: git mv <path> webpresso/blueprints/draft/',
        };
    }
 ...
 */
function validateParkedFolder(status, isInParked) {
    if (status === 'parked' && !isInParked) {
        return {
            valid: false,
            error: 'Plan has status: parked but is not in parked/ folder. Move it with: git mv <path> webpresso/blueprints/parked/',
        };
    }
 ...
 */
export function validatePlanState(markdown, filePath) {
