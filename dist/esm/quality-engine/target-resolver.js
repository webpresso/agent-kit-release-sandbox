...
export const ROOT_LEVEL_PACKAGES = ['infra'];
/** Category queries that match multiple packages */
export const CATEGORY_QUERIES = ['platform', 'admin', 'website'];
/** Directories to skip when scanning for packages */
export const SKIP_DIRECTORIES = ['templates', 'node_modules'];
...
function getResolverWorkspacePackages(deps) {
    ...
    throw new Error('Package resolution requires explicit workspace context. Pass repoRoot or workspacePackages.');
}
function normalizeWorkspacePattern(pattern) {
    return pattern.trim().replace(/^\.\//, '');
}
function readWorkspacePatterns(repoRoot, fs = defaultFs) {
    const workspacePath = join(repoRoot, WORKSPACE_MARKER);
    if (!fs.existsSync(workspacePath)) {
        return undefined;
    }
    try {
        const parsed = parseYaml(fs.readFileSync(workspacePath));
        if (!Array.isArray(parsed?.packages)) {
            return undefined;
        }
        return parsed.packages.filter((value) => typeof value === 'string');
    }
    catch {
        return undefined;
    }
 ...
 */
const COMMON_FLAG_TYPOS = new Map([
    ['fix', '--fix'],
 ...
 */
export function looksLikeFilePath(target) {
 ...
 */
function detectFlagTypo(target) {
 ...
 */
export function findRepoRoot(startDir, fs = defaultFs) {
 ...
 */
export function isFilePath(target, repoRoot, fs = defaultFs) {
 ...
 */
export function generatePathCandidates(target) {
 ...
 */
export function findFirstExistingPath(candidates, repoRoot, fs = defaultFs) {
 ...
 */
export function resolvePartialPath(target, repoRoot, fs = defaultFs) {
 ...
 */
export function readPackageInfo(packagePath, fs = defaultFs) {
 ...
 */
export function getSubdirectoryPackages(baseDir, fs = defaultFs) {
    const packages = [];
    try {
...
function getRecursivePackages(baseDir, fs = defaultFs) {
    const packages = [];
...
function getPackagesForWorkspacePattern(repoRoot, pattern, fs = defaultFs) {
 ...
 */
export function getWorkspacePackages(repoRoot, fs = defaultFs) {
    const packages = [];
    const workspacePatterns = readWorkspacePatterns(repoRoot, fs);
    const patterns = workspacePatterns && workspacePatterns.length > 0
        ? workspacePatterns
 ...
 */
export function matchPackage(pkg, query) {
...
export function isCategoryQuery(query) {
    return CATEGORY_QUERIES.includes(query);
}
function matchCategoryPackage(pkg, query) {
    const shortName = getPackageShortName(pkg.name);
    if (query === 'website') {
        return shortName === 'website';
    }
    return (shortName === `${query}-web` || shortName === `${query}-api` || shortName === `${query}-worker`);
}
/**
 ...
 */
export function parseQueryTokens(target) {
    return target
        .split(',')
 ...
 */
export function findMatchingPackages(packages, query) {
 ...
 */
export function resolvePackageFilters(target, packages) {
 ...
 */
export function resolvePackagePaths(target, packages) {
    return resolveMatchingPackages(target, packages).map((pkg) => pkg.path);
}
function resolveMatchingPackages(target, packages) {
 ...
 */
export function resolveTargetStrict(target, deps = {}) {
    ...
    if (flagCorrection) {
        throw new Error(`Invalid target "${trimmed}" - this looks like a flag argument.\n` +
    ...
    if (filters.length > 0)
        return { type: 'package', value: filters };
    // Not a package - reject
    throw new Error(`Package not found: "${trimmed}". Use --file for file targets.\n` +
 ...
 */
export function resolveCommandTargets(commandName, positionalTargets, options = {}, deps = {}) {
...
function resolvePositionalTarget(target, commandName, deps) {
    ...
    throw new Error(`Ambiguous input: "${target}". Use --package or --file to specify target type.\n` +
        `Example: just ${commandName} --package ${target} or just ${commandName} --file ${target}`);
}
function getSinglePositionalTarget(positionalTargets) {
    ...
    if (normalized.length > 1) {
        throw new Error(`Multiple positional targets are not supported yet: ${normalized.join(', ')}.\n` +
            'Use a single --package or --file flag until multi-target support ships.');
    }
    return normalized[0];
}
function assertNoPositionalMix(positionalTarget, flagLabel, commandName, exampleValue) {
    if (!positionalTarget)
        return;
    const sanitizedValue = exampleValue || '<value>';
    throw new Error(`Cannot combine positional target "${positionalTarget}" with ${flagLabel}.\n` +
 ...
 */
export function normalizeVariadicFlag(flagLabel, values) {
